...
if ~any(EEG.reject.gcompreject)
    ...
    save(fullfile(Source_Comps, Filename), 'EEG')
end
...
if CheckOutput
    ...
    load(fullfile(Source_Cuts, Filename), 'badchans_postICA')
    BC = badchans_postICA;
    ...
    if strcmp(xComp, 'y') % if component selection was ok, see if final product is ok
        ...
        disp('If its mostly fine, but you want to remove a few extra channels, just list them. Negative numbers restore removed channels')
        disp(['bad ch: ', num2str(BC)])
        xEEG = input('Is the EEG ok? (y/n/s/redo/channel list) ');

        % add new bad channels to the list, remove negative numbers
        if isnumeric(xEEG)
            RM = xEEG(xEEG > 0);
            RS = xEEG(xEEG < 0);

            if ~isempty(RM)
                rmCh_postICA(fullfile(Source_Cuts, Filename), RM)
            end

            if ~isempty(RS)
                rsCh_postICA(fullfile(Source_Cuts, Filename), abs(RS))
            end
...
if strcmp(xEEG, 'y') || strcmp(xEEG, 'auto')
    ...
    load(fullfile(Source_Cuts, Filename), 'badchans_postICA') % manually selected bad channels

    NotEEGCh = labels2indexes([EEG_Channels.notEEG, badchans_postICA], NewEEG.chanlocs); % convert based on already removed channels

    NewEEG = pop_select(NewEEG, 'nochannel', NotEEGCh);
...
switch xEEG
    ...
    case 'redo' % end
        ...
        delete(fullfile(Source_Comps, Filename))
        delete(fullfile(Destination, Filename))
        ...
        if ~isempty(badchans_postICA)
            rsCh_postICA(fullfile(Source_Cuts, Filename), badchans_postICA)
        end
...
% rsCh_postICA(fullfile(Source_Cuts, Filename), [])
% CheckOutput = false;
